...
def create_model(X_test, X_train, Y_test,  lrate = 1e-5):
	...
	with tf.name_scope('input'):
		...
		#model['keep-prob'] = tf.placeholder(tf.float32, [], name='keep-prob')
	...
	with tf.name_scope('encoding-layer-2'):
		model['dropout-e'] = tf.nn.dropout(model['ye_1'], 0.5)
		...
		model['ye_2'] = tf.nn.sigmoid(tf.add(tf.matmul(model['dropout-e'], model['We_2']), model['Be_2']), name = 'ye-2')

	
	# FIRST DECODING LAYER

	with tf.name_scope('decoding-layer-1'):
		model['dropout-d'] = tf.nn.dropout(model['ye_2'], 0.5)
		...
		model['yd_1'] = tf.nn.tanh(tf.add(tf.matmul(model['dropout-d'], model['Wd_1']), model['Bd_1']), name = 'yd-1')
...
def train_model(model, X_train, epoch = 100):
	...
	with tf.Session() as session:
		...
		path_model = './model-4-53'
		path_logdir = 'logs-auto-4-53'
...
def test_model(model, X_test, Y_test):
	...
	with open('logfile-4-53.txt','w+') as outfile:
		with tf.Session() as sess:
...
def consfusion_eval(labels, file):
	...
	for i in range(200,400):
		tp, tn, fp, fn = evaluate(labels, file, (i/50000))
		print('Thresh: ', i/50000, 'Conf: ', tp, ' -- ', tn, ' -- ', fp, ' -- ', fn)
...
def main():
	...
	#train_model(model, X_train, epoch = 100)
	#test_model(model, X_test, Y_test)
	consfusion_eval(Y_test, 'logfile-4-53.txt')
