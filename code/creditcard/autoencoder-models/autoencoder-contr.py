...
def create_model(X_test, X_train, Y_test,  lrate = 1e-5):
	...
	with tf.name_scope('loss_optim'):
		...
		model['sq-jac'] = tf.square(tf.reduce_mean(model['contr-1'], axis=1))
		model['frob-jac'] = tf.norm(model['sq-jac'], ord='fro', axis=[1,2])
...
def test_model(model, X_test, Y_test):
	...
	with open('logfile-13.txt','w+') as outfile:
		with tf.Session() as sess:
...
def main():
	...
	consfusion_eval(Y_test, 'logfile-13.txt')
