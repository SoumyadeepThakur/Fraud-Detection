...
def train_model(model, X_train, epoch = 100):
	...
	with tf.Session() as session:
		...
		path_model = './model-3-53'
		path_logdir = 'logs-auto-3-53'
...
def test_model(model, X_test, Y_test):
	...
	with open('logfile-3-53.txt','w+') as outfile:
		with tf.Session() as sess:
...
def main():
	...
	train_model(model, X_train, epoch = 200)
	test_model(model, X_test, Y_test)
	consfusion_eval(Y_test, 'logfile-3-53.txt')
