...
def train_model(model, X_train, epoch = 50):
	...
	with tf.Session() as session:
		...
		path_model = './model-vae-3'
		path_logdir = 'logs-auto-vae-3'
...
def test_model(model, X_test, Y_test):
	...
	with open('logfile-vae-2.txt','w+') as outfile:
...
def consfusion_eval(labels, file):
	...
	for i in range(0,100):
		tp, tn, fp, fn = evaluate(labels, file, i/10.0)
...
def main():
	...
	consfusion_eval(Y_test, 'logfile-vae-2.txt')
